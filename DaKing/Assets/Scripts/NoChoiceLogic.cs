...
public class NoChoiceLogic : IChoiceLogic {
    ...
    public void ExecuteChoice(ExecuteChoices executeChoices)
    {
        ...
        if (CheckIfYouHaveEnoughResources(executeChoices))
        {
            ...
            // Handle Money
            iTween.ValueTo(_thisTransform.gameObject, iTween.Hash("from", _playerAttributes.money, "to", _playerAttributes.money + executeChoices.NoMoneyOutcome, "onupdate", "itweenChangeMoney"));
            _spawnCoins.updateCoins(executeChoices.NoMoneyOutcome);

            // Handle Military
            iTween.ValueTo(_thisTransform.gameObject, iTween.Hash("from", _playerAttributes.military, "to", _playerAttributes.military + executeChoices.NoMilitaryOutcome, "onupdate", "itweenChangeMilitary"));

            // Handle Mood
            int moodOutcome = _playerAttributes.depression + executeChoices.NoMoodOutcome;
            ...
            //Display flash text
            _playerAttributes.flashTextValues(executeChoices.NoMoneyOutcome, executeChoices.NoMilitaryOutcome, executeChoices.NoMoodOutcome);
